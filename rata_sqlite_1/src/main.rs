    ...
    fn submit_message(&mut self) {
        ...
        if bool_add == true {
            ...
            if let Some(stripped) = input_buff.strip_prefix("add ") {
                ...
                //println!("input_buff={}", input_buff);  
            }                
            let c_input = CString::new(input_buff.clone()).unwrap();
            unsafe {
                let lib = Library::new("sample.dll").unwrap();
                
                let todo_add_func: Symbol<extern "C" fn(*const c_char, *mut c_char, c_int) -> c_int> =
                    lib.get(b"todo_add").unwrap();
                let freeString: Symbol<extern "C" fn(*const c_char)> =
                    lib.get(b"free_string").unwrap();

                let mut buffer = vec![0u8; 1024];
                let send_message = CString::new(input_buff.clone()).unwrap();
        ...
        if bool_list == true {
            ...
            unsafe {
                ...
                let todo_list_func: Symbol<extern "C" fn(*const c_char, *mut c_char, c_int) -> c_int> =
                    lib.get(b"todo_list").unwrap();

                let send_message = CString::new("").unwrap();            
                // 受信用バッファ（nバイト確保）
                ...
                // 受信した文字列を表示
                let received = CStr::from_bytes_until_nul(&buffer).unwrap().to_string_lossy();
                //println!("Rust Receive: {}", received);
                let items: Vec<Item> = serde_json::from_str(&received).unwrap();
                for item in items {
                    let row_str: String = item.id.to_string();
                    let s3 = format!("id={} , {}", row_str, item.title);
                    self.messages.push(s3);
                }
                //self.messages.push(received.to_string());
                self.input.clear();
                self.reset_cursor();                
            }            
